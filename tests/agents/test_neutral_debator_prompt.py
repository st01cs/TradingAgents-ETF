"""
Unit tests for neutral debator prompt loading functionality.
"""
import os
import pytest
...
class TestNeutralDebatorPromptLoading:
    """Test suite for neutral debator multi-language prompt loading."""

    def test_load_chinese_prompt(self):
        """Test loading Chinese prompt for neutral debator."""
        prompt = load_prompt_template('neutral_debator', 'zh')
        assert '作为中立风险分析师' in prompt
        assert '平衡的观点' in prompt  # Actual phrase in translation
        assert '适度风险策略' in prompt
        assert '风险分析师' in prompt
        assert '保守分析师' in prompt

    def test_load_english_prompt(self):
        """Test loading English prompt for neutral debator."""
        prompt = load_prompt_template('neutral_debator', 'en')
        assert 'As the Neutral Risk Analyst' in prompt
        assert 'balanced perspective' in prompt
        assert 'moderate risk strategy' in prompt
    ...
    def test_default_language_is_chinese(self):
        ...
        try:
            ...
            prompt = load_prompt_template('neutral_debator')
            assert '作为中立风险分析师' in prompt
            assert '平衡的观点' in prompt  # Actual phrase in translation
        finally:
    ...
    def test_prompt_file_not_found(self):
        ...
        with pytest.raises((FileNotFoundError, ValueError)) as exc_info:
            load_prompt_template('neutral_debator', 'fr')
    ...
    def test_invalid_language_code(self):
        ...
        with pytest.raises(ValueError) as exc_info:
            load_prompt_template('neutral_debator', 'invalid')
    ...
    def test_chinese_terminology_consistency(self):
        ...
        # Check key terminology (aligned with risky debator)
        assert '中立风险分析师' in prompt
        assert '风险分析师' in prompt
        assert '保守分析师' in prompt
        assert '对话记录' in prompt

    def test_citation_format_consistency(self):
        """Test that citation format is consistent with risky debator."""
        prompt = load_prompt_template('neutral_debator', 'zh')
        assert '报告' in prompt  # Check for report citations
        # Should match risky debator format

    def test_neutral_style_preserved(self):
        """Test that neutral debating style is preserved in Chinese prompt."""
        prompt = load_prompt_template('neutral_debator', 'zh')

        # Check for neutral debating language
        assert '平衡' in prompt or '适度' in prompt
        assert '集双方优点' in prompt or '极端波动' in prompt
    ...
    def test_chinese_prompt_structure(self):
        ...
        assert '辩论' in prompt or '论证' in prompt
